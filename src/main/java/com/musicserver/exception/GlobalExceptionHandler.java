    ...
    public Result<Void> handleAuthenticationException(AuthenticationException e, HttpServletRequest request) {
        ...
        return switch (e) {
            case BadCredentialsException ignored ->
                    Result.error(ResultCode.INVALID_USERNAME_OR_PASSWORD.getCode(), "用户名或密码错误");
            case DisabledException ignored ->
                    Result.error(ResultCode.USER_DISABLED.getCode(), "用户账户已被禁用");
            case LockedException ignored -> Result.error(ResultCode.USER_LOCKED.getCode(), "用户账户已被锁定");
            default -> Result.error(ResultCode.UNAUTHORIZED.getCode(), "认证失败");
        };
    }
